...
// ===================== CONTACT AURA ENGINE =====================
// Matches Mission Log aura exactly: same particle counts, sizes, timing, and density

function ContactAuraEngine() {
  // Sparks - small bright dots flying outward (24 particles - matches Mission Log)
  ...
  // Particle blobs - soft, large, drifting (10 particles - matches Mission Log)
  ...
  // Embers - medium bright, fade to nothing (16 particles - matches Mission Log)
  ...
  // Energy waves - concentric rings expanding outward (4 waves - matches Mission Log)
  ...
  // Heat distortion shimmer layers (3 layers - matches Mission Log)
  ...
  return (
    <div aria-hidden="true" className="pointer-events-none absolute inset-0 z-10 flex items-center justify-center overflow-visible">
      {/* Wide outer glow - intensified (matches Mission Log) */}
      <div
        className="absolute"
        style={{
          width: "750px",
...
function ContactItem({ icon: Icon, label, value, href, delay, mounted }: ContactItemProps) {
  ...
  return (
    <a
      ...
      onClick={handleClick}
      className={`group relative flex items-center gap-4 rounded-sm border border-transparent px-4 py-3 transition-all duration-500 hover:border-danger/20 hover:bg-danger/[0.03] ${
    ...
    >
      ...
      <div
        aria-hidden="true"
        className="absolute inset-0 rounded-sm opacity-0 transition-opacity duration-300 group-hover:opacity-100"
        style={{
          boxShadow: "inset 0 0 20px rgba(200,50,45,0.08), 0 0 15px rgba(200,50,45,0.05)",
        }}
      />

      {/* Icon */}
      <div className="relative flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-danger/20 bg-danger/[0.05] transition-all duration-300 group-hover:scale-110 group-hover:border-danger/40 group-hover:bg-danger/[0.08]">
        <Icon className="h-4 w-4 text-danger/60 transition-colors duration-300 group-hover:text-danger/80" />
      </div>
      ...
      <div className="flex flex-col gap-0.5">
        ...
        <span className="font-mono text-xs tracking-wide text-foreground/70 transition-colors duration-300 group-hover:text-danger/80">
          {value}
        </span>
      </div>
      ...
      <div className="ml-auto opacity-0 transition-all duration-300 group-hover:translate-x-1 group-hover:opacity-100">
        <ChevronRight className="h-4 w-4 text-danger/40" />
      </div>
    </a>
  )
}
  ...
  return (
    ...
    >
        ...
        >
          ...
          >
            ...
            <div className="relative px-8 py-8 sm:px-10 sm:py-10">
              ...
              {/* Tagline */}
              <p
                className={`mt-5 font-mono text-[10px] uppercase tracking-[0.2em] text-danger/50 transition-all duration-700 sm:text-[11px] ${
                  mounted ? "translate-x-0 opacity-100" : "-translate-x-4 opacity-0"
                }`}
              ...
              <div className="mt-6 flex flex-col gap-2">
                ...
                <ContactItem
                  icon={Github}
                  label="GitHub"
                  value="github.com/biswasadrisha70-ops"
      ...
      >
        ...
        <button
          onClick={handlePrev}
          className="group flex cursor-pointer items-center justify-center border border-danger/20 bg-[#0a0a0f]/70 p-3 backdrop-blur-sm transition-all duration-400 hover:border-danger/40 hover:bg-danger/[0.06] active:scale-95"
          style={{
